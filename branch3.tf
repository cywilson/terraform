...
resource "aws_vpc" "vpc_branch3" {
  cidr_block                                         = "10.111.0.0/16"
  provider                                                             = aws.branch3
  tags = {
    Name = "Branch1 VPC"
  }
}
...
resource "aws_subnet" "branch3_vpc_subnet1" {
    vpc_id                                                              = aws_vpc.vpc_branch3.id
...
resource "aws_subnet" "branch3_vpc_subnet2" {
    vpc_id                                                              = aws_vpc.vpc_branch3.id
...
resource "aws_internet_gateway" "branch3_vpc_igw" {
    vpc_id = aws_vpc.vpc_branch3.id
    tags = {
